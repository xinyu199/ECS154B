...
class PipelinedCPU(implicit val conf: CPUConfig) extends BaseCPU {
  ...
  val id_opcode = if_id.io.data.instruction(6,0)
  control.io.opcode := id_opcode
  ...
  //ex_mem.io.in.mem_writedata := id_ex.io.data.readdata2
