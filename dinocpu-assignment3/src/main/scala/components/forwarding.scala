...
class ForwardingUnit extends Module {
  ...
  // Your code goes here
  when(io.exmemrd === io.rs1 && io.rs1 =/= 0.U && io.exmemrw === true.B){
    io.forwardA := 1.U
  }
  .elsewhen(io.memwbrd === io.rs1 && io.rs1 =/= 0.U && io.memwbrw === true.B){
    io.forwardA := 2.U
  }
  .otherwise{
    io.forwardA := 0.U
  ...
  when(io.exmemrd === io.rs2 && io.rs2 =/= 0.U && io.exmemrw === true.B){
    io.forwardB := 1.U
  }
  .elsewhen(io.memwbrd === io.rs2 && io.rs2 =/= 0.U && io.memwbrw === true.B){
    io.forwardB := 2.U
  }
  .otherwise{
    io.forwardB := 0.U
